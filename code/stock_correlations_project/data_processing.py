...
Style: Write in present tense, as if you went back in time to the end of the given year and quarter and wrote the business descriptio at that time. The business model description must be extremely concise, objective, and focused. Use the minimum number of tokens necessary to convey the essential details. The description must not exceed 300 words. Do not include a title but use subtitles to clearly separate different aspects of the business model. 
...
Revenue Model: Description of how the company generates revenue and its primary revenue sources, such as product sales or recurring revenue. Do not use company specific product names.
...
Geographic Regions: The main geographic regions where the company operates or generates significant revenue. Just answer with the two most significant regions.
Risk: Description of the 2 key risk factors. Don't mention competition.
Industry Value-Drivers: Describe the top 3 industry-wide value-driving factors.
Short Investment Case: As of the end of the given year and quarter, what are bearish investors' arguments for shorting the stock?
Buy Investment Case: As of the end of the given year and quarter, what are bullish investor's arguments for buying the stock?
...
def fetch_and_store_company_descriptions(openai_model: OpenaiModel, batch_ids_file_path: str,
                                        preexisting_file_path: str = r"C:\repos\Deep-learning-trj\data\processed_data_storage\company_descriptions\company_descriptions.parquet",
    ...
    new_data_df = openai_model.store_batch_results(batch_results=batch_results, is_chat_completion=True, custom_id_col="ticker_region_calendar_quarter",
                                                content_col="company_description", preexisting_file_path=preexisting_file_path, storage_file=storage_file_path)
...
def process_data_create_tensors_and_mapping_df(example_companies=["AAL-US", "MSFT-US", "AAPL-US", "HIMS-US"], cut_off_date="1999-12"):
    ...
    if price_key_padding_mask is not None:
        data_utils.store_tensor(price_key_padding_mask, r"C:\repos\Deep-learning-trj\data\processed_data_storage\stock_correlations_project\example_companies\prices_key_padding_mask")
    ...
    if fundamentals_key_padding_mask is not None:
        data_utils.store_tensor(fundamentals_key_padding_mask, r"C:\repos\Deep-learning-trj\data\processed_data_storage\stock_correlations_project\example_companies\fundamentals_key_padding_mask")
    ...
    if missing_features_key_padding_mask is not None:
        data_utils.store_tensor(missing_features_key_padding_mask, r"C:\repos\Deep-learning-trj\data\processed_data_storage\stock_correlations_project\example_companies\missing_features_fundamentals_key_padding_mask")
    ...
    if macro_key_padding_mask is not None:
        data_utils.store_tensor(macro_key_padding_mask, r"C:\repos\Deep-learning-trj\data\processed_data_storage\macro_data\macro_key_padding_mask")
    ...
    data_utils.store_tensor(company_descriptions_input_tensor, r"C:\repos\Deep-learning-trj\data\processed_data_storage\stock_correlations_project\example_companies\company_descriptions_input_tensor", dtype='<i8')
    data_utils.store_tensor(company_descriptions_key_padding_mask, r"C:\repos\Deep-learning-trj\data\processed_data_storage\stock_correlations_project\example_companies\company_descriptions_key_padding_mask", dtype='<i8')

    target_tensor, _ = data_utils.convert_iterable_to_tensor(target_df["corr_dotproduct"])
    data_utils.store_tensor(target_tensor, r"C:\repos\Deep-learning-trj\data\processed_data_storage\stock_correlations_project\example_companies\target_tensor")
    mapping_df.to_parquet(r"C:\repos\Deep-learning-trj\data\processed_data_storage\stock_correlations_project\example_companies\mapping_df.parquet")

    return mapping_df

if __name__ == "__main__":

    example_companies = ["AAL-US", "MSFT-US", "AAPL-US", "HIMS-US"]
   
    # create_processed_data(["AAL-US", "MSFT-US", "AAPL-US", "HIMS-US"])
    ...
    # target_data = pd.read_parquet(r"C:\repos\Deep-learning-trj\data\processed_data_storage\stock_correlations_project\example_companies\target_data.parquet")
    ...
    # response = openai_model.retrieve_batch("batch_675d482881f08190bb8ef3a18eb44f58")
    # print(response)
    ...
    # user_message["content"] = user_message["content"].format("Entra", "ENTRA-NO", "Q1", "2022")
