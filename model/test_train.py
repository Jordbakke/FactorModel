...
def shuffle_split_load_dataset( dataset: Dataset, train_size: float, test_size: float, val_size: float, batch_size: int, num_workers: int, collate_fn=None, shuffle: bool = True, random_seed: int = 42):
...
def evaluate_and_save(model, optimizer, val_dataloader, train_losses, val_losses, loss_fn, device="cpu", 
                      model_val_loss=float('inf'), scheduler=None, max_val_batches=10000, save_path=None):
    ...
    # Save the best model
    if val_loss < model_val_loss and save_path:
...
def train_and_evaluate(train_dataloader, val_dataloader, loss_fn, model=None, optimizer = None, scheduler=None,
                       existing_model_path=None, device="cpu", num_epochs=1, eval_frequency=1000,
        ...
        for i, batch in enumerate(tqdm(train_dataloader, desc="Training Batches")):
            ...
            if (i + 1) % eval_frequency == 0:
                ...
                val_losses, model_val_loss = evaluate_and_save(model=model, optimizer=optimizer, val_dataloader=val_dataloader,
                                                               ...
                                                               device=device, model_val_loss=model_val_loss, scheduler=scheduler, max_val_batches=max_val_batches, save_path=save_path)
        # End-of-epoch evaluation
        if (i + 1) % eval_frequency != 0:
            avg_train_loss = accumulated_train_loss / (i + 1)
            train_losses.append(avg_train_loss)
            val_losses, model_val_loss = evaluate_and_save(model, optimizer, val_dataloader, train_losses, val_losses,
                                        loss_fn, device, model_val_loss, save_path=save_path)
