...
def create_tensorstore_mapping(tensor_df: pd.DataFrame, tensor_df_reference_name: str, tensor_column: str, left_on: list, right_on: list,
                               period_column: str = None, period_frequency: str = "M", mapping_df_path: str = None,
                               mapping_df_storage_path: str = None, ffill=False, ffill_limit: int = 0) -> pd.DataFrame:
    ...
    if set(right_on).difference(set(tensor_df.columns)):
        raise ValueError("id_cols must be a subset of tensor_df columns")

    tensor_df = tensor_df.dropna(subset=[tensor_column, *right_on])
    ...
    if mapping_df is None:
        mapping_df = tensor_df[right_on + [tensor_df_reference_name]]
    
    mapping_df = mapping_df.merge(tensor_df, left_on=left_on, right_on=right_on, how="outer")

    if ffill and ffill_limit and period_column:
        mapping_df = add_missing_periods(mapping_df, period_column=period_column, freq=period_frequency, insert_entity=True, entity_columns=right_on)
        group_cols = [id_col for id_col in right_on if id_col != period_column]
        mapping_df = mapping_df.groupby(group_cols, group_keys=False)[mapping_df.columns].apply(lambda group: group.sort_values(
        by=period_column, ascending=True).ffill(limit=ffill_limit)).reset_index(drop=True)
...
def from_tensor_df_to_mapped_tensorstore(tensor_df: pd.DataFrame, tensor_df_reference_name: str,
                                         tensor_column: str, left_on: list, right_on: list, period_column: str = None,
                                         period_frequency: str = "M",mapping_df: pd.DataFrame = None, mapping_df_storage_path: str = None,
                                         ffill=False, ffill_limit: int = 0, target_sequence_length=None, pad_value=0, tensorstore_path: str = None):
    
    mapping_df = create_tensorstore_mapping(tensor_df = tensor_df, tensor_df_reference_name = tensor_df_reference_name, tensor_column = tensor_column,
                                            left_on = left_on, right_on = right_on, period_column = period_column, period_frequency=period_frequency,
                                            mapping_df = mapping_df, mapping_df_storage_path = mapping_df_storage_path, ffill=ffill, ffill_limit=ffill_limit)
...
if __name__ == "__main__":
    ...
    corporate_debt["release_month"] = corporate_debt["year_quarter"].apply(lambda x: pd.Period(x.end_time, freq="M") + 2)
    ...
    corporate_debt = corporate_debt[["release_month", "corporate_debt_growth", "year_quarter"]]
    corporate_debt = expand_time_series(corporate_debt, "year_quarter", "year_month")
    ...
    core_inflation["release_month"] = core_inflation["year_month"] + 1
    core_inflation = core_inflation[["year_month","release_month", "core_inflation"]]

    df = release_date_aware_merge_and_sequence_creation([corporate_debt, core_inflation], calendar_period_col="year_month", release_period_col="release_month", sequence_length=12, sequence_column_name="previous_macros")
