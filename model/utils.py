...
class MaskedNetwork(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, num_feature_hidden_layers, output_dim,
                 activation_function=nn.GELU, dropout_prob=0.1, dropout_layer_frequency=2, num_masked_hidden_layers=2):
        
        """
        Class for a masked network that takes two inputs: a feature vector and a masked vector.
        The feature vector and masked vector are processed by two separate feedforward neural networks and the outputs are concatenated.
        """
        
        super(MaskedNetwork, self).__init__()
        
        self.feature_network = FFNN(embedding_dim=embedding_dim, hidden_dim=hidden_dim, num_hidden_layers=num_feature_hidden_layers,
        ...
        self.masked_network = FFNN(embedding_dim=embedding_dim, hidden_dim=hidden_dim, num_hidden_layers=num_masked_hidden_layers,
                                   output_dim=output_dim, activation_function=activation_function, dropout_prob=dropout_prob,
                                   dropout_layer_frequency=dropout_layer_frequency)

    def forward(self, x, mask):
        feature_output = self.feature_network(x)
        masked_output = self.masked_network(mask)
        # Concatenate along the last dimension
        return torch.cat([feature_output, masked_output], dim=-1)
...
class HeadCombinationLayer(nn.Module):
    ...
    def __init__(self, input_dims: list, num_ffnn_hidden_layers: int, final_dim: int, num_heads: int, activation_function=nn.GELU, dropout_prob=0.1,
        ...
        for input_dim in input_dims:
            self.linear_projections.append(FFNN(embedding_dim=input_dim, hidden_dim=final_dim, num_hidden_layers=num_ffnn_hidden_layers, output_dim=final_dim,
...
if __name__ == "__main__":
    ...
    a = HeadCombinationLayer(input_dims=[2, 4], final_dim=3, num_heads=3, num_ffnn_hidden_layers=2)
    result = a(t1, t2)
    print(result)
    print(result.shape)
