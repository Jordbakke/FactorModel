...
def release_date_aware_processing_and_sequence_creation(df: pd.DataFrame, calendar_period_col: str, release_period_col: str,
    ...
    Should be used when data for the same calendar timestamp have different release dates. Should only be used for small dataframes. 
    """
    ...
    df = df.sort_values(by=[calendar_period_col])
    ...
    for i, release_period in enumerate(sorted(df[release_period_col].unique(), reverse=True)):
        ...
        if i == 0: # last release period
            if cols_to_standardize:
                global_means = available_data_df[cols_to_standardize].mean()
                global_stds = available_data_df[cols_to_standardize].std(ddof=0)
        ...
        if cols_to_standardize:
            available_data_df[cols_to_standardize] = (available_data_df[cols_to_standardize] - global_means) / global_stds
        ...
        available_data_df = pd.DataFrame({release_period_col: [release_period], new_column_name: [available_data_df[value_columns].values[-sequence_length:, :]]})
