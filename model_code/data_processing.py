...
def process_macro_data(macro_directory=r"C:\repos\Deep-learning-trj\data\raw_data_storage\macro_data",
    ...
    def process_helper(file_path: str, date_col: str, cols_to_standardize: list = None, pd_read_func=pd.read_csv,
                        ...
                        agg_func_groupby_col: str = None,
                        ratio_log_transform_cols: list = None, ffill=False,
                        ffill_limit=1, month_offset: int=None, destination_path: str = None) -> pd.DataFrame:
        ...
        if month_offset: #quarterly data from Fed is start month of the quarter
            df["calendar_month"] = df["calendar_month"] + month_offset
    ...
    "US_corporate_debt_growth": {
        ...
        "ffill": True,
        "ratio_log_transform_cols": ["corporate_debt"],
        "month_offset": 2,
    ...
    "US_real_gdp": {
        ...
        "is_quarterly_data": True,
        "ffill": True,
        "ratio_log_transform_cols": ["real_gdp"],
        "month_offset": 2,  # No aggregation, pct_change computed later
    ...
    real_interest_rates["10y_yield"] = pd.to_numeric(real_interest_rates["10y_yield"], errors="coerce").astype(float)
    real_interest_rates["core_inflation"] = pd.to_numeric(core_inflation["core_inflation"], errors="coerce").astype(float)
    real_interest_rates = data_utils.add_missing_periods(real_interest_rates, "calendar_month", "M").ffill(limit=1)
    ...
    oil_prices = data_utils.standardize_columns(oil_prices, cols_to_standardize=["brent_inflation_adjusted", "wti_inflation_adjusted"])
