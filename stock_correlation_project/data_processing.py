...
Cost Structure: What are the company's 3 most important cost drivers? Write about the services, components and material the company uses to provide its own product or service rather than broad cost categories, such as "operating expenses" or "manufacturing costs". Be granular and list the cost components as as bullet points.
Target Market: Granular decription of target market(s) and customer segments. Include sub market segments if applicable, e.g 'deep water drilling' or 'shallow water drilling' instead of just 'drilling'. Do not use company specific product names.
...
Strengths: What are the key strengths of the company? List them as bullet points.
Weaknesses: What are the key weaknesses of the company? List them as bullet points.
"""
}
}
...
def process_data_create_tensors_and_mapping_df(example_companies=["AAL-US", "MSFT-US", "AAPL-US", "HIMS-US"], cut_off_date="1999-12"):
    ...
    data_utils.store_tensor(prices_tensor, r"C:\repos\Deep-learning-trj\stock_correlation_project\test_train_data\prices_tensor")
    if price_key_padding_mask is not None:
        data_utils.store_tensor(price_key_padding_mask, r"C:\repos\Deep-learning-trj\stock_correlation_project\test_train_data\prices_key_padding_mask")
    ...
    data_utils.store_tensor(company_descriptions_input_tensor, r"C:\repos\Deep-learning-trj\stock_correlation_project\test_train_data\company_descriptions_input_tensor", dtype='<i8')
...
if __name__ == "__main__":
    ...
    # user_message["content"] = user_message["content"].format("Microsoft", "MSFT-US", "Q2", "2023")
