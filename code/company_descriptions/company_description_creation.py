...
Style: Write in present tense, as if you went back in time to the end of the given year and quarter and wrote the business descriptio at that time. The business model description should be extremely concise, objective, and to the point.
...
Sector and supply chain position: Which sector does the company operate in and where does it stand in the supply chain?
...
class CompanyDescriptionWriter:
    ...
    def get_company_description_example(self, company_name, ticker_region, year, quarter, message_key="business_model_generator"):
        
        user_message = USER_MESSAGES[message_key].copy()
        ...
        try:
            ...
            response = openai.chat.completions.create(
                ...
                messages=[SYSTEM_MESSAGES[message_key], user_message],
    ...
    def create_embedding(self, company_description: str):
        embedding = client.embeddings.create(
        input=company_description, model=writer.openai_embedding_model, dimensions=writer.embedding_dim).data[0].embedding
    ...
    def get_batch_results(self, batch_ids_file_path:str ,
                               batch_result_file_path=r"C:\repos\Deep-learning-trj\data\code\company_descriptions\openai_api_json_file\batch_job_results.jsonl",
                               ):
        """
    ...
    def create_company_description_request_units(self, company_description_units: list, user_message_key: str, system_message_key: str) -> list:
        ...
        for company_description_unit in company_description_units:
            ...
            request_body =  {"model": self.openai_model, "messages": messages,"max_tokens": self.max_output_tokens,
...
def encode_company_descriptions_huggingface(company_descriptions: pd.DataFrame, model_name="bert-base-uncased", storage_path = None):
    """
    Function that encodes company descriptions using a Hugging Face model
    """
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    encoded_descriptions = company_descriptions["company_description_encoded"].apply(lambda x: tokenizer(x, return_tensors="pt"))
    
    if storage_path is not None:
        encoded_descriptions.to_parquet(storage_path)
...
if __name__== "__main__":
    ...
    description1 = writer.get_company_description_example("Cadeler", "CADLR-NO", "2023", "Q4", message_key="business_model_generator")
    print(description1)
    
    description2 = writer.get_company_description_example("Transocean", "RIG-US", "2021", "Q4", message_key="business_model_generator")
    print(description2)

    # description1 = writer.get_company_description_example("Amazon", "AMZN-US", "2023", "Q2", message_key="business_model_generator")
    # print(description1)
