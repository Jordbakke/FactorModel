...
SYSTEM_MESSAGES = {"business_model_generator": {"role": "system", "content": """You are an expert stock portfolio risk analyst. When provided with a company name and a specific year, your job is to generate a business model description for the provided company as of the end of the specified year.
Describe only what was known up to that time, with absolutely no information or insights from beyond that year.
Style: Write in past tense and it is EXTREMELY important that you describe the business as it was at that time. Use the minimum number of tokens necessary to convey the details. The language must be simple, clear, and straightforward, focusing only on concrete information. Avoid stylistic or descriptive phrases. The description cannot exceed 300 words
...
"business_model_generator": {"role": "user", "content": """Describe the business model of {}, ticker {}, as of the end of {}. The business model description shall include the following aspects:
...
Business Area: What were the company's business area(s) in {}? List the business area as bullet points. If it's just one , write it as a single bullet point. Ignore insignificant, non-core business areas.
...
Cost Structure: What were the company's 3 most important cost drivers in {}? Write about the services, components and materials the company used to provide its own product(s) or service(s) rather than broad cost categories, such as "manufacturing costs". When applicable, name the specific commodities and components. List the cost drivers as as bullet points.
...
Strengths: In {}, what were the key strengths of the business? It is extremely important that you do not hallucinate or make up strengths. Be strictly factual. List them as bullet points. 
Weaknesses: In {}, what were the key weaknesses of the business? It is extremely important that you do not hallucinate or make up weaknesses. Be strictly factual. List them as bullet points.
"""
}
}
...
def process_fundamentals(fundamentals_raw_df: pd.DataFrame,
    ...
    fundamentals_df["missing_features_mask"] = missing_features_masks
...
def process_prices(prices_raw_df: pd.DataFrame, sequence_length = 60, index_ticker="SPY-US") -> pd.DataFrame:
    ...
    lambda group: data_utils.collect_previous_values(
        ...
        allow_nan_in_collected_values=False
    )).reset_index(drop=True)
...
def process_macro_data(macro_directory=r"C:\repos\Deep-learning-trj\data\raw_data_storage\macro_data",
                       ...
                       release_month_column_name: str = "release_month", missing_feature_mask=True):

    def process_helper(date_col: str, df: pd.DataFrame = None, file_path: str = None, pd_read_func=pd.read_csv,
                        rename_cols: dict = None, is_quarterly_data=False, is_daily_data=False,
                        log_ratio_transform_cols=None, cols_to_standardize=None, cutoff_month: str="1990-01"
                        ) -> pd.DataFrame:
        ...
        if is_quarterly_data:
            ...
            df = data_utils.add_missing_periods(df, "calendar_quarter", "Q")
        ...
        if log_ratio_transform_cols:
            df[log_ratio_transform_cols] = df[log_ratio_transform_cols].apply(pd.to_numeric, errors='coerce').astype(float)
            df = data_utils.log_ratio_transform(df=df, cols_to_transform=log_ratio_transform_cols, inplace=True, drop_original_cols=True)
        if cols_to_standardize:
            df[cols_to_standardize] = df[cols_to_standardize].apply(pd.to_numeric, errors='coerce').astype(float)
            df = data_utils.standardize_columns(df, cols_to_standardize=cols_to_standardize, inplace=True)
    ...
    "US_corporate_debt": {
        ...
        "log_ratio_transform_cols": ["corporate_debt"],
    },
    "EBIT_US_companies": {
    ...
    "US_real_gdp": {
        ...
        "is_quarterly_data": True,
        "log_ratio_transform_cols": ["real_gdp"],
    },
    "US_consumer_sentiment": {
        ...
        "cols_to_standardize": ["consumer_sentiment"]
    }
    }
    ...
    sp_data = process_helper(date_col="date", df=sp_data)
    ...
    sp_data = data_utils.log_ratio_transform(sp_data, cols_to_transform=["price"], inplace=True, drop_original_cols=True)
    sp_data = data_utils.standardize_columns(sp_data, cols_to_standardize=["pe_3m_lagged_earnings"], inplace=True)
    ...
    treasury_yield = process_helper(date_col="date", file_path=os.path.join(macro_directory, "10y_treasury_yield.csv"),
                                    rename_cols={"DGS10": "10y_yield"}, is_daily_data=True).drop(columns=["release_month"])
    core_inflation = process_helper(date_col="date", file_path=os.path.join(macro_directory, "US_core_inflation.csv"),
                                    rename_cols={"CORESTICKM159SFRBATL": "core_inflation"}).drop(columns=["release_month"])
    ...
    macro_df = data_utils.release_date_aware_merge_and_sequence_creation(dfs=all_macro_dfs, calendar_period_col="calendar_month", release_period_col=release_month_column_name,
                                                                         sequence_column_name="macro_sequence", sequence_length=sequence_length, missing_feature_mask=missing_feature_mask)
    return macro_df
...
def process_commodity_data(macro_directory=r"C:\repos\Deep-learning-trj\data\raw_data_storage\macro_data",
    ...
    us_cpi = data_utils.add_missing_periods(us_cpi, "calendar_month", "M")
    ...
    commodity_df = data_utils.standardize_columns(commodity_df, cols_to_standardize=numeric_cols)
    commodity_df["commodity_values"] = commodity_df[numeric_cols].apply(lambda row: np.nan if row.isna().any() else row.tolist(), axis=1)
    commodity_df = data_utils.collect_previous_values(df=commodity_df, value_column="commodity_values", date_column="calendar_month",
                                                        new_column_name="commodity_sequence", include_current_row=True, drop_empty_sequences=True,
                                                        sequence_length=sequence_length, allow_nan_in_collected_values=False)
...
def process_target_data(prices_df: pd.DataFrame, periods=3, sequence_length: int = 4,
                        arima_time_series: int = 1000, random_state=42,
                        arima_storage_path: str=r"C:\repos\Deep-learning-trj\stock_correlation_project\test_train_data\arima_data.parquet",
                        process_arima_data=False) -> pd.DataFrame:
    ...
    #create arima data
    if process_arima_data:
        target_df["ticker_region1_ticker_region2"] = target_df["ticker_region1"] + "_" + target_df["ticker_region2"]
        unique_combinations = target_df["ticker_region1_ticker_region2"].unique()
        np.random.seed(random_state)
        random_sample = np.random.choice(unique_combinations, size=min(arima_time_series, len(unique_combinations)), replace=False)
        arima_data = target_df[target_df["ticker_region1_ticker_region2"].isin(random_sample)]
        arima_data = arima_data[["calendar_month", "ticker_region1", "ticker_region2", "correlation"]].copy()
        arima_data = process_arima_data(arima_data=arima_data, sequence_length=sequence_length, storage_path=arima_storage_path)
        target_df = target_df.drop(columns=["ticker_region1_ticker_region2"])
    
    target_df = data_utils.standardize_columns(target_df, cols_to_standardize=["correlation"])
    target_df = target_df[['calendar_month', 'ticker_region1','ticker_region2', 'correlation']]

    if process_arima_data:
        return target_df, arima_data
    else:
        return target_df, None

def process_arima_data(arima_data: pd.DataFrame, sequence_length: int, storage_path: str = None) -> pd.DataFrame:

    arima_data = data_utils.standardize_grouped_columns(arima_data, group_columns="ticker_region1_ticker_region2", cols_to_standardize=["correlation", "dot_product"])
    arima_data["row_number"] = arima_data.groupby("ticker_region1_ticker_region2").cumcount()
    arima_data = arima_data[arima_data["row_number"] % sequence_length == 0].drop(columns="row_number")
...
def create_company_descriptions_batch_job(openai_model: OpenaiModel, target_df: pd.DataFrame, company_names_df: pd.DataFrame,
    ...
    company_description_df["calendar_year"] = company_description_df["calendar_month"].dt.to_timestamp().dt.to_period("Y")
    ...
    #add description for the year before the earliest target data year for each ticker as description from the end of December from previous year is used. 
...
def process_data_create_tensors_and_mapping_df(example_companies=["RIG-US", "MSFT-US", "AAPL-US", "VAL-US"], cut_off_date="1999-12"):
    ...
    for col in ["ticker_region1", "ticker_region2"]:
        ...
        mapping_df = data_utils.create_tensor_mapping(mapping_df=mapping_df, tensor_df = processed_fundamentals, tensor_df_reference_name="fundamentals_" + col,
                                                      ...
                                                      right_on=["release_month", "ticker_region"], how="outer", apply_add_missing_periods=True,
...
if __name__ == "__main__":
    ...
    fundamentals = pd.read_parquet(r"C:\repos\Deep-learning-trj\data\raw_data_storage\company_data\largest_5000_US_tickers_by_year_shares_only\fundamentals\fundamentals.parquet")
    fundamentals = fundamentals[fundamentals["ticker_region"].isin(example_companies)]
    f = process_fundamentals(fundamentals[fundamentals["ticker_region"].isin(example_companies)])
    data_utils.open_in_excel(f)
    # prices_raw = pd.read_csv(r"C:\repos\Deep-learning-trj\data\raw_data_storage\company_data\largest_5000_US_tickers_by_year_shares_only\prices\prices.csv")
    # prices_raw = prices_raw[prices_raw["ticker_region"].isin(example_companies + ["SPY-US"])]
    # target_data = process_target_data(prices_raw, periods=3, sequence_length=4)
    ...
    # response = openai_model.retrieve_batch("batch_67667d95d6c08190bf242e7cfa049e3a")
    # print(response)
    ...
    # user_message["content"] = custom_format(USER_MESSAGES["business_model_generator"]["content"], "Microsoft", "MSFT-US", "2007")
    ...
    # data_utils.open_in_excel(pd.read_csv(r"C:\repos\Deep-learning-trj\data\raw_data_storage\macro_data\US_corporate_debt_growth.csv"))
    # data_utils.open_in_excel(pd.read_excel(r"C:\repos\Deep-learning-trj\data\raw_data_storage\macro_data\robert_schiller_data.xlsx"))
    # data_utils.open_in_excel(pd.read_csv(r"C:\repos\Deep-learning-trj\data\raw_data_storage\macro_data\US_real_gdp.csv"))
    # data_utils.open_in_excel(pd.read_csv(r"C:\repos\Deep-learning-trj\data\raw_data_storage\macro_data\EBIT_US_companies.csv"))
