...
class HuggingFaceModel:
    ...
    def encode_text_huggingface(self, text, truncation=True, padding=True):

        """
        Function that encodes company descriptions using a Hugging Face model
        """
        tokenizer = AutoTokenizer.from_pretrained(self.model_name)
        encoded_text = tokenizer(text, return_tensors="pt", truncation=truncation, padding=padding)
        input_ids = torch.tensor(encoded_text["input_ids"].tolist())
        attention_mask = torch.tensor(encoded_text["attention_mask"].tolist())
        
        return input_ids, attention_mask
    
    def create_encoded_df(self, text_df: pd.DataFrame, text_col: str, truncation=True, padding=True, drop_text_col=False):

        text_df["input_ids"] = text_df[text_col].apply(lambda x: self.encode_text_huggingface(x, truncation=truncation, padding=padding)[0])
        text_df["attention_mask"] = text_df[text_col].apply(lambda x: self.encode_text_huggingface(x, truncation=truncation, padding=padding)[1])

        if drop_text_col:
            text_df.drop(columns=[text_col], inplace=True)
