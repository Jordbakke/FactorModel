...
def standardize_columns(df, cols_to_standardize=None, inplace=False) -> pd.DataFrame:
...
def standardize_rolling_window(df, window_size, cols_to_standardize=None, inplace=False) -> pd.DataFrame:
...
def standardize_time_windows(df, period_col, period_freq, num_periods, cols_to_standardize=None, include_current_period=False, min_periods=1) -> pd.DataFrame:
    """
...
def log_ratio_transform(df, cols_to_transform: list, shift_value=1, inplace=False, new_column_names: list = None,
                        drop_original_cols=False) -> pd.DataFrame:
...
def add_missing_timestamps( df: pd.DataFrame, period_column: str, freq: str, release_period_column= None, release_period_column_offset = 0, entity_columns: list = None, include_padding_mask=False,
    ...
    if entity_columns:
        df[entity_columns] = df[entity_columns].fillna(method='ffill')
    ...
    if include_padding_mask:
        ...
        df[padding_mask_col] = is_missing.map({True: 1, False: 0})
        ...
        if pad_value is not None:
            ...
            df.loc[df[padding_mask_col] == 1, numeric_cols] = pad_value
    ...
    if include_missing_features_mask:
        ...
        df[missing_features_mask_col] = df[missing_features_mask_col].combine_first(
...
def convert_df_to_sequences(df, value_columns: list, date_column, new_column_name, padding_mask_column: str = None,
    ...
    if padding_mask_column is not None:
        padding_mask_series = df[padding_mask_column].to_numpy()
        padding_masks = []
    
    if missing_features_mask_column is not None:
        missing_features_mask_series = df[missing_features_mask_column].to_numpy()
...
def release_date_aware_merge_and_sequence_creation(dfs: list, calendar_period_col: str, release_period_col: str,
                                                      sequence_column_name: str, sequence_length):
    ...
    for i, release_period in enumerate(sorted(merged_df[release_period_col].unique(), reverse=True)):
        ...
        available_data_df["missing_feature_mask"] = available_data_df[numeric_cols].isna().astype(float).to_numpy().tolist()

        available_data_df = available_data_df.ffill()

        if i == 0:
            global_means = available_data_df[numeric_cols].mean()
            global_stds = available_data_df[numeric_cols].std(ddof=0)
            missing_periods = get_missing_periods(available_data_df[calendar_period_col], freq="M")
            missing_periods_df = pd.DataFrame({calendar_period_col: missing_periods, "padded": [1] * len(missing_periods),
        ...
        available_data_df["padded"] = 0
        available_data_df = available_data_df.merge(missing_periods_df, on=calendar_period_col, how="outer")
        available_data_df["padded"] = available_data_df["padded_x"].combine_first(available_data_df["padded_y"])
        available_data_df["padded"] = available_data_df["padded"].astype(int)
        available_data_df["missing_feature_mask"] = available_data_df["missing_feature_mask_x"].combine_first(available_data_df["missing_feature_mask_y"])
        available_data_df = available_data_df.drop(columns=["padded_x", "padded_y", "missing_feature_mask_x", "missing_feature_mask_y"])
        ...
        release_date_aware_dfs.append(pd.DataFrame({release_period_col: [release_period],
                                                    ...
                                                    "padded": [available_data_df["padded"].tolist()[-sequence_length:]],
                                                    "missing_feature_mask": [available_data_df["missing_feature_mask"].tolist()[-sequence_length:]]}))

    return pd.concat(release_date_aware_dfs, axis=0)

def expand_tensor_to_3d(tensor: torch.tensor):

    if tensor.dim() > 3:
        raise ValueError("Tensors must be 3D or lower")
...
def convert_iterable_to_tensor(iterable, target_sequence_length=None, sequence_pad_value=0,
    ...
    if target_sequence_length is None:
        tensors = [expand_tensor_to_3d(torch.tensor(tensor, dtype=tensor_dtype)) for tensor in iterable]
        tensors = torch.cat(tensors, dim=0)
        assert len(tensors.shape) == 3, "Tensors must be 3D"

        return tensors, None
    else:
        tensors_and_masks = [pad_or_slice_and_create_key_padding_mask(expand_tensor_to_3d(torch.tensor(tensor, dtype=tensor_dtype)), target_sequence_length,
...
def create_tensor_mapping(mapping_df: pd.DataFrame, tensor_df: pd.DataFrame, tensor_df_reference_name: str, tensor_column: str,
    ...
    if apply_add_missing_timestamps and period_col and freq: #useful when data is not available for all periods so that several different periods should refer to the same data. E.g data released every third month.
...
if __name__ == "__main__":
    ...
    data = {
    "calendar_month": [
        pd.Period("1990-01", freq="M"),
        pd.Period("1990-01", freq="M"),
        pd.Period("1990-03", freq="M"),
        pd.Period("1990-03", freq="M"),
    ],
    "release_month": [
        pd.Period("1990-01", freq="M"),
        pd.Period("1990-02", freq="M"),
        pd.Period("1990-04", freq="M"),
        pd.Period("1990-05", freq="M"),
    ],
    "10y_2y_spread": [-0.938108277, np.nan, -1.089882916, np.nan],
    "10y_yield": [2.12909478, np.nan, 2.240593478, np.nan],
    "core_inflation": [np.nan, 1.792902974, np.nan, 2.046371699],
    }

    # df = pd.DataFrame(data)
    
    # df["missing_features_mask"] = df[["10y_2y_spread", "10y_yield", "core_inflation"]].isna().astype(float).to_numpy().tolist()

    # df = add_missing_timestamps(df, "calendar_month", freq="M", include_padding_mask=True, pad_value=0, release_period_column="release_month", include_missing_features_masks=True, missing_features_masks_col="missing_features_mask")
    # open_in_excel(df)  
    # df = release_date_aware_merge_and_sequence_creation([df], "calendar_month", "release_month", "sequence", 3)

    # open_in_excel(df)

    # missing_periods = get_missing_periods(df["calendar_month"], freq="M")
    # print(missing_periods)

    a = list(get_missing_periods(periods=[pd.Period("2020-01"), pd.Period("2020-02"), pd.Period("2020-03")], freq="M"))
    print(type(a))
    print(a)
