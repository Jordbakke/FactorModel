...
def pad_or_slice_sequence(tensor: torch.tensor, target_sequence_length: int, pad_value=0, pre_pad=False):

    if not tensor.dim() == 3 and not tensor.dim() == 2:
        raise ValueError("Only 2D or 3D tensors are supported.")
    
    sequence_length = tensor.shape[1] if tensor.dim == 3 else tensor.shape[0]
...
def expand_or_slice_padding_mask(padding_mask: torch.Tensor, target_sequence_length: int, pre_pad=False, pad_value=1) -> torch.Tensor:
    ...
    if not padding_mask.dim() == 2 and not padding_mask.dim() == 1:
...
def convert_iterable_to_tensor(iterable, pad_or_slice_func=None, target_sequence_length=None, pad_value=0,
                               pre_pad=False, tensor_dtype=torch.float32):
    
    """
    Pads and slices tensors in the iterable and then stacks them into a single tensor.
    """
    if pad_or_slice_func is None:
        tensors = [torch.tensor(tensor, dtype=tensor_dtype) for tensor in iterable]
    else:
        tensors = [pad_or_slice_func(torch.tensor(tensor, dtype=tensor_dtype), target_sequence_length,
                                        pad_value=pad_value, pre_pad=pre_pad) for tensor in iterable]
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame({"A": [[[1,2,3], [4,5,6]]], "mask": [[0,1]]})
    
    print(convert_iterable_to_tensor(df["A"], target_sequence_length=4, pad_value=0, pre_pad=False, tensor_dtype=torch.float32, pad_or_slice_func=pad_or_slice_sequence))
    print(convert_iterable_to_tensor(df["mask"], target_sequence_length=4, pad_value=1, pre_pad=False, tensor_dtype=torch.float32, pad_or_slice_func=expand_or_slice_padding_mask))

    # tensor = convert_iterable_to_tensor(df["A"], target_sequence_length=4, pad_value=0, pre_pad=False, tensor_dtype=torch.float32)
    # print(tensor)
