...
def add_missing_periods(df: pd.DataFrame, period_column: str, freq: str, 
                        include_padding_mask=True, pad_value=None, entity_columns=None) -> pd.DataFrame:
    """
    ...
    -------
    pd.DataFrame
        DataFrame with missing periods (and entities) filled in as NaN rows and a 'padded' column.
    """
    ...
    sorted_periods = df[period_column].sort_values()
    min_period = sorted_periods.iloc[0]
    max_period = sorted_periods.iloc[-1]

    missing_periods = get_periods_between(
        min_period, max_period, include_start_period=True, include_end_period=True, freq=freq
    )

    missing_periods_df = pd.DataFrame({period_column: missing_periods})
    # Merge to include missing periods and entities
    result = pd.merge(
        missing_periods_df, df, on=[period_column], 
        how="outer", suffixes=('_missing', '')
    ).reset_index(drop=True)
    
    if entity_columns:
        result[entity_columns] = result[entity_columns].fillna(method='ffill')
    
    if include_padding_mask:
        result['padded'] = result[period_column].isin(df[period_column]).map({True: False, False: True})
        if pad_value is not None:
            result.loc[result["padded"] == True] = result.loc[result["padded"] == True].fillna(pad_value)
...
def convert_df_to_sequences(df, value_columns: list, seq_mask_column, date_column, new_column_name, sequence_length=None, include_current_row=False,
                   drop_empty_sequences=False, direction="previous", drop_original_columns=False) -> pd.DataFrame:
    ...
    seq_masks = df[seq_mask_column].to_numpy()
    ...
    df["sequence_mask"] = sequence_masks

    # Optionally drop empty sequences
    if drop_empty_sequences:
        df = df[df[new_column_name].apply(lambda x: not np.all(np.isnan(x)))]

    if drop_original_columns:
        df = df.drop(columns=value_columns + [seq_mask_column])
    return df
...
def release_date_aware_merge_and_sequence_creation(dfs: list, calendar_period_col: str, release_period_col: str,
                                                      ...
                                                      missing_feature_mask: bool = False):
    ...
    for release_period in merged_df[release_period_col].unique():
        ...
        if missing_feature_mask: #add missing feature mask
            available_data_df["missing_feature_mask"] = available_data_df[numeric_cols].isna().astype(float).values.tolist()

        available_data_df = available_data_df.ffill()

        available_data_df["value_list"] = available_data_df[numeric_cols].values.tolist()

        if missing_feature_mask:
...
def create_tensor_mapping(mapping_df: pd.DataFrame, tensor_df: pd.DataFrame, tensor_df_reference_name: str, tensor_column: str,
    ...
    if apply_add_missing_periods and period_col and freq: #useful when data is not available for all periods so that several different periods should refer to the same data. E.g data released every third month.
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame({"date": [pd.Period("2021-01", freq="M"), pd.Period("2021-05", freq="M"), pd.Period("2021-08", freq="M")], "entity": ["A", "A", "A"],
                       "val": [1,3, 2], "val2": [10, 30, 20]})
    print(df)
    r = df.groupby("entity", group_keys=False)[df.columns].apply(lambda group: add_missing_periods(group, "date", "M", True, 0, ["entity"]))
    print(r)
    r = convert_df_to_sequences(df=r, date_column="date", new_column_name="val_sequence", value_columns=["val", "val2"], seq_mask_column="padded", sequence_length=3, include_current_row=True, drop_empty_sequences=True, direction="previous")
    print(r)