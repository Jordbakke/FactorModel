...
from ml_code.data_processing.llm_utils import OpenaiModel, HuggingFaceModel
...
def process_fundamentals(fundamentals_raw_df: pd.DataFrame,
    ...
    missing_features_masks = fundamentals_df[numeric_cols].isna().astype(float)
    missing_features_masks.columns = [col + "_missing" for col in missing_features_masks.columns]
    missing_features_cols = missing_features_masks.columns.tolist()
    ...
    fundamentals_df = pd.concat([fundamentals_df, missing_features_masks], axis=1) #concatenate missing_feature_mask horizontally
    ...
    lambda group: data_utils.add_missing_timestamps(
        df=group, period_column="calendar_quarter", freq="Q", entity_columns=["ticker_region"], include_padding_mask=True, padding_mask_col="padding_mask",
                        pad_value=0, value_cols=numeric_cols, missing_features_cols = missing_features_cols, missing_features_pad_value=1
    ), include_groups=True
    ...
    fundamentals_df = fundamentals_df.groupby("ticker_region", group_keys=False)[fundamentals_df.columns].apply(lambda group: data_utils.add_sequence_to_df(
                                        df=group, value_columns=fundamentals_df.select_dtypes(include=['number']).columns.tolist(),
                                        sequence_length=sequence_length, new_column_name="fundamentals_sequence",
                                        convert_to_1d=False, date_column="calendar_quarter",
                                        include_current_row=True,
                                        drop_empty_sequences=True, direction="previous", drop_value_columns=False)
                                        ).reset_index(drop=True)
    
    fundamentals_df = fundamentals_df.groupby("ticker_region", group_keys=False)[fundamentals_df.columns].apply(lambda group: data_utils.add_sequence_to_df(
                                        df=group, value_columns=["padding_mask"],
                                        sequence_length=sequence_length, new_column_name="padding_mask",
                                        convert_to_1d=True, date_column="calendar_quarter",
                                        include_current_row=True,
                                        drop_empty_sequences=True, direction="previous", drop_value_columns=False)
                                        ).reset_index(drop=True) 
    
    fundamentals_df = fundamentals_df[["calendar_quarter", "release_month", "ticker_region", "fundamentals_sequence", "padding_mask"]]
...
def process_prices(prices_raw_df: pd.DataFrame, sequence_length = 60, index_ticker="SPY-US") -> pd.DataFrame:
    ...
    prices_df = prices_df.merge(index_prices, on=["calendar_month"], how="left")
    prices_df.dropna(inplace=True)

    prices_df = prices_df.groupby("ticker_region")[prices_df.columns].apply(lambda group: data_utils.add_sequence_to_df(df=group, value_columns=["log_adj_price_ratio", "index_log_adj_price_ratio"],
                                              sequence_length=sequence_length, new_column_name="stock_index_log_adj_price_ratios_sequence",
                                              convert_to_1d=False, date_column="calendar_month", include_current_row=True,
                                              drop_empty_sequences=True, direction="previous", drop_value_columns=False)).reset_index(drop=True)

    return prices_df[["calendar_month", "release_month", "ticker_region", "stock_index_log_adj_price_ratios_sequence"]]
...
def process_macro_data(macro_directory=r"C:\repos\Deep-learning-trj\data_storage\raw_data_storage\macro_data", 
    ...
    sp_data = pd.read_excel(os.path.join(r"C:\repos\Deep-learning-trj\data_storage\raw_data_storage\macro_data", "robert_schiller_data.xlsx"))
    ...
    macro_df = data_utils.release_date_aware_processing_and_sequence_creation(df=macro_df, calendar_period_col="calendar_month",
                                                                        ...
                                                                        "unemployment_rate", "consumer_sentiment", "pe_3m_lagged_earnings", "real_interest_rate"],
                                                                         ...
                                                                         log_transform_shift_value=1, include_current_row=True
                                                                         )
    

    return macro_df

def process_commodity_data(macro_directory=r"C:\repos\Deep-learning-trj\data_storage\raw_data_storage\macro_data",
                       cpi_file_path = r"C:\repos\Deep-learning-trj\data_storage\raw_data_storage\macro_data\US_CPI.csv", sequence_length = 60,
                       release_month_column_name: str = "release_month"):
    ...
    commodity_df = data_utils.add_sequence_to_df(df=commodity_df, value_columns=numeric_cols, date_column="calendar_month",
...
def create_processed_data(example_companies = ["AAL-US", "MSFT-US", "AAPL-US", "HIMS-US"]):
    ...
    prices_raw = pd.read_csv(r"C:\repos\Deep-learning-trj\data_storage\raw_data_storage\company_data\largest_5000_US_tickers_by_year_shares_only\prices\prices.csv")
    ...
    fundamentals = pd.read_parquet(r"C:\repos\Deep-learning-trj\data_storage\raw_data_storage\company_data\largest_5000_US_tickers_by_year_shares_only\fundamentals\fundamentals.parquet")
    ...
    company_features_raw = pd.read_csv(r"C:\repos\Deep-learning-trj\data_storage\raw_data_storage\company_data\largest_5000_US_tickers_by_year_shares_only\company_features\company_features.csv")
...
def process_data_create_tensors_and_mapping_df(example_companies=["RIG-US", "MSFT-US", "AAPL-US", "VAL-US"], cut_off_date="1999-12"):
    ...
    #prices
    prices = pd.read_csv(r"C:\repos\Deep-learning-trj\data_storage\raw_data_storage\company_data\largest_5000_US_tickers_by_year_shares_only\prices\prices.csv")
...
if __name__ == "__main__":
    ...
    example_companies = ["MSFT-US", "AAPL-US"]
    ...
    # company_names_df = pd.read_csv(r"C:\repos\Deep-learning-trj\data_storage\raw_data_storage\company_data\largest_5000_US_tickers_by_year_shares_only\company_features\company_features.csv")
