...
def standardize_time_windows(df, period_col, period_freq, num_periods, cols_to_standardize=None, include_current_period=False, min_periods=1):
    ...
    if not isinstance(df[period_col], pd.Period):
    ...
    for period in unique_periods:
        start_period = max(min(unique_periods), period - num_periods + offset)
...
def log_ratio_transform(df, cols_to_transform: list, inplace=False, new_column_names: list = None):
    ...
    if not inplace:
        ...
        transformed_df[new_column_names] = np.log(transformed_df[cols_to_transform] / transformed_df[cols_to_transform].shift(1))
        return transformed_df
    else:
        df[new_column_names] = np.log(df[cols_to_transform] / df[cols_to_transform].shift(1))
        return df
...
def add_year_month_column(df: pd.DataFrame, date_column: str, year_month_column_name:str = None) -> pd.DataFrame:
    ...
    if year_month_column_name:
        df = df.rename(columns={"year_month": year_month_column_name})
    return df

def add_year_quarter_column(df: pd.DataFrame, date_column: str, year_quarter_column_name:str = None) -> pd.DataFrame:
    # Ensure the date column is in datetime format
    if not pd.api.types.is_datetime64_any_dtype(df[date_column]):
        df[date_column] = pd.to_datetime(df[date_column])
    ...
    if year_quarter_column_name:
        df = df.rename(columns={"year_quarter": year_quarter_column_name})
    return df
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame({"date": ["2021-01", "2021-01", "2021-02", "2021-03"], "value1": [2, 1, 4, 3], "value": [1, 2, 3, 4]})

    print(standardize_time_windows(df, period_col="date", period_freq="M", num_periods=2, cols_to_standardize=["value", "value1"], include_current_period=False))